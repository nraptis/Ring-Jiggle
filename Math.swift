...
struct Math {
    ...
    public static func angleToVector(radians: Float) -> SIMD2<Float> {
        .init(sinf(radians), -cosf(radians))
    }
    
    public static func face(target: SIMD2<Float>) -> Float {
        -atan2f(Float(-target.x), Float(-target.y))
    }
    
    public static func rotatePoint(point: SIMD2<Float>, radians: Float) -> SIMD2<Float> {
        var dist = point.x * point.x + point.y * point.y
        if dist > epsilon {
            dist = sqrtf(dist)
            let pivot = face(target: point)
            let newDir = angleToVector(radians: pivot + radians)
            return SIMD2<Float>(newDir.x * dist, newDir.y * dist)
        }
        return point
    }
    
    public static func transformPoint(point: SIMD2<Float>, scale: Float, rotation: Float) -> SIMD2<Float> {
        var x = point.x
        var y = point.y
        if scale != 1.0 {
            x *= scale
    ...
    public static func transformPoint(point: SIMD2<Float>, translation: SIMD2<Float>, scale: Float, rotation: Float) -> SIMD2<Float> {
        var result = transformPoint(point: point, scale: scale, rotation: rotation)
        result = SIMD2<Float>(x: result.x + translation.x, y: result.y + translation.y)
        return result
    }
    
    public static func untransformPoint(point: SIMD2<Float>, scale: Float, rotation: Float) -> SIMD2<Float> {
        transformPoint(point: point, scale: 1.0 / scale, rotation: -rotation)
    }
    
    public static func untransformPoint(point: SIMD2<Float>, translation: SIMD2<Float>, scale: Float, rotation: Float) -> SIMD2<Float> {
        var result = SIMD2<Float>(x: point.x - translation.x, y: point.y - translation.y)
