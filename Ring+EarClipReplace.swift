...
//  Jiggle3
//
//  Created by Nicky Taylor on 2/5/24.
//
...
extension Ring {
    ...
    func earClipReplaceRingPointZeroRingPoints(ringPointIndex: Int) -> Bool {
        ...
        partsFactory.depositRingLineSegment(previousLineSegmentRight)
    ...
    func earClipReplaceRingPointTwoRingPoints(ringPointIndex: Int) -> Bool {
        ...
        var dumbList = [RingPoint]()
        
        for i in 0..<ringPointCount {
            
            if i == ringPointIndex {
                
                let point1 = partsFactory.withdrawRingPoint()
                let point2 = partsFactory.withdrawRingPoint()
                
                point1.x = neighborLeft.x + deltaX * 0.35
                point1.y = neighborLeft.y + deltaY * 0.35
                
                point2.x = neighborLeft.x + deltaX * 0.65
                point2.y = neighborLeft.y + deltaY * 0.65
                
                dumbList.append(point1)
                dumbList.append(point2)
            } else {
                let point1 = partsFactory.withdrawRingPoint()
                point1.x = ringPoints[i].x
                point1.y = ringPoints[i].y
        ...
        attemptCalculateLineSegments()
        
        attemptCalculateRingPointNormals()
        
        attemptCalculateRingPointAngularSpans()
        */
