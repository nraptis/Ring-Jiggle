    ...
    func calculateLineSegmentPointNeighbors() {
        if ringPointCount > 0 {
            ...
            guard ringLineSegmentCount == ringPointCount else {
                print("[G87] Fatal: ringLineSegmentCount \(ringLineSegmentCount) should be ringPointCount \(ringPointCount).")
                return
            }
    ...
    func calculateProbeSegmentPointNeighbors() {
        if ringProbePointCount > 0 {
            ...
            guard ringProbeSegmentCount == ringProbePointCount else {
                print("[G98] Fatal: ringProbeSegmentCount \(ringProbeSegmentCount) should be ringProbePointCount \(ringProbePointCount).")
                return
            }
